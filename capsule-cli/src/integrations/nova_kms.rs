...
const DEFAULT_KMS_REQUEST_TIMEOUT_MS: u64 = 10_000;
...
impl NovaKmsProxy {
    pub fn new(config: &KmsIntegration, capsule_api_endpoint: String) -> Result<Self> {
    ...
    async fn local_eth_address(&self) -> Result<String> {
        let value = self.capsule_api_get("/v1/eth/address").await?;
        let address = value
            .get("address")
            .and_then(Value::as_str)
            .ok_or_else(|| anyhow!("Capsule API /v1/eth/address response missing address"))?;
    ...
    async fn local_sign_message(&self, message: &str) -> Result<String> {
        ...
        let signature = value
            .get("signature")
            .and_then(Value::as_str)
            .ok_or_else(|| anyhow!("Capsule API /v1/eth/sign response missing signature"))?;
        Ok(signature.to_string())
    }

    async fn local_tee_pubkey(&self) -> Result<String> {
        let value = self.capsule_api_get("/v1/encryption/public_key").await?;
        let pubkey = value
            .get("public_key_der")
            .and_then(Value::as_str)
            .ok_or_else(|| {
                anyhow!("Capsule API /v1/encryption/public_key response missing public_key_der")
            })?;
        Ok(trim_0x(pubkey))
    }
    ...
    async fn encrypt_payload_for_node(
        ...
        let nonce = value
            .get("nonce")
            .and_then(Value::as_str)
            .ok_or_else(|| anyhow!("Capsule API /v1/encryption/encrypt response missing nonce"))?;
        let encrypted_data = value
            .get("encrypted_data")
            .and_then(Value::as_str)
            .ok_or_else(|| {
                anyhow!("Capsule API /v1/encryption/encrypt response missing encrypted_data")
            })?;
    ...
    async fn decrypt_envelope(
        ...
        let plaintext = value
            .get("plaintext")
            .and_then(Value::as_str)
            .ok_or_else(|| anyhow!("Capsule API decrypt response missing plaintext"))?;
        Ok(serde_json::from_str(plaintext)?)
    }

    async fn capsule_api_get(&self, path: &str) -> Result<Value> {
        let url = format!("{}{}", self.capsule_api_endpoint, path);
        let (client, client_route) = self.http_client_for_url(&url);
        info!(
            "Nova KMS CAPSULE_API GET path={} url={} client_route={}",
            path, url, client_route
        );

        let response = client.get(&url).send().await.map_err(|err| {
            anyhow!(
                "capsule_api GET transport failed path={} url={}: {}",
                path,
                url,
                err
            )
        ...
        if !status.is_success() {
            ...
            warn!(
                "Nova KMS CAPSULE_API GET failed path={} url={} client_route={} http={} body={}",
                path,
                url,
                client_route,
                status.as_u16(),
            ...
            bail!(
                "capsule_api GET failed path={} url={} http={} body={}",
                path,
                url,
                status.as_u16(),
                body_preview
        ...
        serde_json::from_str::<Value>(&response_body).map_err(|err| {
            anyhow!(
                "capsule_api GET invalid JSON path={} url={}: {}",
                path,
                url,
                err
            )
    ...
    async fn capsule_api_post(&self, path: &str, payload: &Value) -> Result<Value> {
        let url = format!("{}{}", self.capsule_api_endpoint, path);
        let (client, client_route) = self.http_client_for_url(&url);
        info!(
            "Nova KMS CAPSULE_API POST path={} url={} client_route={} payload={}",
            path,
            url,
            client_route,
        ...
        let response = client
            ...
            .map_err(|err| {
                anyhow!(
                    "capsule_api POST transport failed path={} url={}: {}",
                    path,
                    url,
                    err
                )
        ...
        if !status.is_success() {
            let body_preview = preview_text_for_log(&response_body);
            warn!(
                "Nova KMS CAPSULE_API POST failed path={} url={} client_route={} http={} body={}",
                path,
                url,
                client_route,
                status.as_u16(),
            ...
            bail!(
                "capsule_api POST failed path={} url={} http={} body={}",
                path,
                url,
                status.as_u16(),
                body_preview
        ...
        serde_json::from_str::<Value>(&response_body).map_err(|err| {
            anyhow!(
                "capsule_api POST invalid JSON path={} url={}: {}",
                path,
                url,
                err
            )
...
mod tests {
    ...
    fn test_proxy() -> NovaKmsProxy {
        NovaKmsProxy {
            ...
            capsule_api_endpoint: "http://127.0.0.1:18000".to_string(),
    ...
    async fn spawn_mock_capsule_api_server(
        state: MockCapsuleApiState,
            ...
            loop {
                ...
                tokio::spawn(async move {
                    ...
                    let service = service_fn(move |req: Request<Incoming>| {
                        ...
                        async move {
                            Ok::<_, Infallible>(handle_mock_capsule_api_request(state, req).await)
                        }
                    });
    ...
    async fn handle_mock_capsule_api_request(
        state: Arc<MockCapsuleApiState>,
    ...
    #[tokio::test]
    async fn smoke_test_call_kms_on_node_with_mock_capsule_api_and_kms() {
        ...
        let capsule_api_state = MockCapsuleApiState {
            app_private_key_hex,
            app_wallet,
            app_tee_pubkey,
            node_tee_pubkey: node_tee_pubkey.clone(),
        ...
        let (capsule_api_base_url, capsule_api_handle) =
            spawn_mock_capsule_api_server(capsule_api_state).await;
        let (kms_base_url, kms_handle) = spawn_mock_kms_server(kms_state).await;

        let mut proxy = test_proxy();
        proxy.capsule_api_endpoint = capsule_api_base_url;
