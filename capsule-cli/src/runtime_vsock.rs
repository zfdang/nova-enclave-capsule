use crate::constants::{
    CAPSULE_MANAGED_CID_END, CAPSULE_MANAGED_CID_START, HOST_RUNTIME_CLOCK_SYNC_OFFSET,
...
pub fn allocate_managed_enclave_cid(used: &HashSet<u32>) -> Result<u32> {
    for cid in CAPSULE_MANAGED_CID_START..=CAPSULE_MANAGED_CID_END {
    ...
    bail!(
        ...
        CAPSULE_MANAGED_CID_START,
        CAPSULE_MANAGED_CID_END
    )
}
...
mod tests {
    use super::*;
    use crate::constants::{
        CAPSULE_MANAGED_CID_START, HOST_RUNTIME_VSOCK_PORT_BASE, HOST_RUNTIME_VSOCK_PORT_STRIDE,
    };

    #[test]
    fn allocate_managed_enclave_cid_skips_used_values() {
        let used = HashSet::from([CAPSULE_MANAGED_CID_START, 99]);
        let cid = allocate_managed_enclave_cid(&used).unwrap();
        assert_eq!(cid, CAPSULE_MANAGED_CID_START + 1);
    }

    #[test]
    fn runtime_ports_for_first_managed_cid_are_deterministic() {
        let ports = RuntimeHostVsockPorts::for_cid(CAPSULE_MANAGED_CID_START).unwrap();
        let block_base = HOST_RUNTIME_VSOCK_PORT_BASE
            + (CAPSULE_MANAGED_CID_START * HOST_RUNTIME_VSOCK_PORT_STRIDE);
    ...
    fn runtime_ports_accept_upper_managed_cid() {
        assert!(RuntimeHostVsockPorts::for_cid(CAPSULE_MANAGED_CID_END).is_ok());
    }

    #[test]
    fn runtime_port_blocks_for_distinct_cids_do_not_overlap() {
        let first = RuntimeHostVsockPorts::for_cid(CAPSULE_MANAGED_CID_START).unwrap();
        let second = RuntimeHostVsockPorts::for_cid(CAPSULE_MANAGED_CID_START + 1).unwrap();
