...
const ALLOW_WEAK_RNG_FALLBACK_ENV: &str = "CAPSULE_RUNTIME_ALLOW_WEAK_RNG_FALLBACK";
            ...
            Some(nsm_ref) => match Self::collect_random_bytes(nsm_ref, 32).and_then(|bytes| {
                ...
                Err(err) => {
                    if Self::weak_rng_fallback_enabled() {
                        log::warn!(
                            "Failed to derive Ethereum key from NSM RNG; CAPSULE_RUNTIME_ALLOW_WEAK_RNG_FALLBACK is enabled, falling back to OsRng: {}",
                            err
                        );
                        Arc::new(EthKey::new())
                    } else {
        ...
        let encryption_key = match nsm.as_ref() {
                ...
                .and_then(|bytes| EncryptionKey::from_entropy(&bytes))
            {
                ...
                Err(err) => {
                    if Self::weak_rng_fallback_enabled() {
                        log::warn!(
                            "Failed to derive P-384 key from NSM RNG; CAPSULE_RUNTIME_ALLOW_WEAK_RNG_FALLBACK is enabled, falling back to OsRng: {}",
                            err
                        );
...
#[async_trait]
impl HttpHandler for CapsuleApiHandler {
...
fn test_collect_random_bytes_from_source_collects_partial_chunks() {
    ...
    let bytes =
        CapsuleApiHandler::collect_random_bytes_from_source(4, || chunks.next().unwrap()).unwrap();
    assert!(bytes == vec![1u8, 2, 3, 4]);
}

#[test]
fn test_collect_random_bytes_from_source_rejects_repeated_empty_chunks() {
    let result = CapsuleApiHandler::collect_random_bytes_from_source(1, || Ok(Vec::new()));
    assert!(result.is_err());
    assert!(
        result
            .unwrap_err()
...
async fn test_attestation_handler() {
    ...
    let handler =
        CapsuleApiHandler::new(Box::new(StaticAttestationProvider::new(Vec::new())), None).unwrap();
