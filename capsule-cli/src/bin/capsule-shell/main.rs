use anyhow::Result;
use capsule_cli::constants::{EIF_FILE_NAME, MANIFEST_FILE_NAME, RELEASE_BUNDLE_DIR};
use capsule_cli::manifest::load_manifest_raw;
use capsule_cli::nitro_cli::NitroCLI;
use capsule_cli::run::{Enclave, EnclaveExitStatus, EnclaveOpts};
use capsule_cli::utils;
...
const CAPSULE_SHELL_INTERRUPTED: u8 = 109;

#[derive(Debug, Parser)]
#[clap(
    author,
    version = env!("CAPSULE_CLI_VERSION_WITH_GIT"),
    about,
    long_about = None
)]
struct Cli {
...
impl Termination for CLISuccess {
    fn report(self) -> ExitCode {
        ...
        match self {
            ...
            EnclaveStatus(Cancelled) => ExitCode::from(CAPSULE_SHELL_INTERRUPTED),
            Ok => ExitCode::SUCCESS,
        }
    }
}
