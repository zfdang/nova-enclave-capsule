...
use capsule_cli::manifest::S3EncryptionMode;
use log::info;
use std::sync::Arc;
...
use capsule_cli::http_util::HttpServer;
use capsule_cli::integrations::nova_kms::NovaKmsProxy;
use capsule_cli::nsm::{Nsm, NsmAttestationProvider};

pub struct CapsuleApiService {
    task: Option<JoinHandle<()>>,
}

impl CapsuleApiService {
    pub async fn start(config: &Configuration, nsm: Arc<Nsm>) -> Result<Self> {
        let task = if let Some(port) = config.api_port() {
            info!("Starting Capsule API on port {port}");
            let capsule_api_endpoint = format!("http://127.0.0.1:{port}");
            ...
            let s3_proxy = if let Some(s3_config) = config.s3_config() {
                ...
                let http_client =
                    capsule_cli::integrations::aws_util::new_proxied_client(proxy_uri.clone())?;
                let imds =
                    capsule_cli::integrations::aws_util::imds_client_with_proxy(proxy_uri).await?;

                let aws_config =
                    capsule_cli::integrations::aws_util::load_config_from_imds(imds).await?;
                ...
                Some(Arc::new(capsule_cli::integrations::s3::S3Proxy::new(
                    client,
            ...
            let nova_kms = if let Some(kms_config) = config.kms_integration_config() {
                ...
                let proxy = Arc::new(NovaKmsProxy::new(kms_config, capsule_api_endpoint)?);
            ...
            let handler = CapsuleApiHandler::with_integrations(
