...
use capsule_cli::constants::{HTTP_EGRESS_PROXY_PORT, MANIFEST_FILE_NAME};
use capsule_cli::manifest::{HeliosRpcKind, Manifest};
...
impl Configuration {
    ...
    pub async fn load<P: AsRef<Path>>(config_dir: P) -> Result<Self> {
        ...
        let manifest = capsule_cli::manifest::load_manifest(&manifest_path).await?;
    ...
    pub fn load_blocking<P: AsRef<Path>>(config_dir: P) -> Result<Self> {
        ...
        let manifest = capsule_cli::manifest::load_manifest_sync(&manifest_path)?;
        Ok(Self::from_manifest(config_dir.as_ref(), manifest))
    }
    ...
    pub fn s3_config(&self) -> Option<&capsule_cli::manifest::S3StorageConfig> {
        self.manifest
            .storage
            .as_ref()
    ...
    pub fn kms_integration_config(&self) -> Option<&capsule_cli::manifest::KmsIntegration> {
        self.manifest
            .kms_integration
            .as_ref()
    ...
    pub fn clock_sync_config(&self) -> Option<capsule_cli::manifest::ClockSync> {
...
mod tests {
    use super::*;
    use capsule_cli::constants::KMS_REGISTRY_HELIOS_PORT;
    use capsule_cli::manifest::{
        AuxApi, CapsuleApi, ClockSync, Egress, HeliosRpc, HeliosRpcChain, Sources,
    };
    ...
    fn base_config() -> Configuration {
        Configuration {
            ...
            manifest: Manifest {
                ...
                egress: None,
                defaults: None,
                api: Some(CapsuleApi { listen_port: 9000 }),
                aux_api: None,
                storage: None,
    ...
    fn aux_api_port_with_max_api_and_explicit_value_does_not_overflow() {
        let mut cfg = base_config();
        cfg.manifest.api = Some(CapsuleApi {
            listen_port: u16::MAX,
        });
        cfg.manifest.aux_api = Some(AuxApi {
