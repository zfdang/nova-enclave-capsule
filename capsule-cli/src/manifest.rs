...
pub struct Sources {
    pub app: String,
    #[serde(rename = "capsule-runtime")]
    pub capsule_runtime: Option<String>,
    #[serde(rename = "capsule-shell")]
    pub capsule_shell: Option<String>,
}
...
pub struct S3StorageConfig {
    ...
    /// Capsule Runtime will automatically ensure this ends with a trailing slash.
    pub prefix: String,
...
fn validate_manifest_cross_constraints(manifest: &Manifest) -> Result<()> {
    if manifest.aux_api.is_some() && manifest.api.is_none() {
        bail!("aux_api requires api.listen_port because Aux API proxies the Capsule API");
    }
    ...
    if let Some(api) = manifest.api.as_ref() {
        ...
        if aux_port == api.listen_port {
            bail!(
                "aux_api.listen_port must differ from api.listen_port because Aux API and the Capsule API cannot share a port"
            );
        }
    }
