...
pub struct AuxApiHandler {
    capsule_api_url: String,
    client: HttpClient,
}

impl AuxApiHandler {
    pub fn new(capsule_api_port: u16) -> Self {
        let capsule_api_url = format!("http://127.0.0.1:{}", capsule_api_port);
    ...
    async fn proxy_request(
        ...
        let uri = format!("{}{}", self.capsule_api_url, path);
        ...
        match self.client.request(req).await {
            Ok(resp) => {
                ...
                // Copy headers from Capsule API response
            ...
            Err(_) => {
                let response = Response::builder()
                    ...
                    .body(Full::new(Bytes::from(
                        r#"{"error":"Capsule API service unavailable"}"#,
                    )))?;
                Ok(response)
            }
        }
    ...
    async fn handle_attestation(&self, body: Bytes) -> Result<Response<Full<Bytes>>> {
        // Parse JSON body and filter only public_key (user_data is allowed, eth_addr will be injected by the Capsule API handler)
        let sanitized_body = match serde_json::from_slice::<Value>(&body) {
            Ok(mut json_value) => {
                ...
                // user_data is allowed - the Capsule API handler will inject eth_addr into it
