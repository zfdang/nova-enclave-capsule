# Nova Enclave Capsule API

Nova Enclave Capsule exposes localhost-only HTTP APIs inside the enclave. One of the main
...
- Primary API: full `/v1/*` surface from `capsule-cli/src/capsule_api.rs`
- Aux API: a restricted proxy surface from `capsule-cli/src/aux_api.rs`
...
```json
{
  "error": "Capsule API service unavailable"
}
```
